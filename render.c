...
/*   Updated: 2025/03/06 02:19:44 by abouguri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void clear_screen(t_data *img, t_cub *data, int window_width, int window_height)
{
...
void   initialize_ray(int x, int window_width, t_cub *data, t_dda *dda, double *ray_dir_x, double *ray_dir_y)
{
...
void calculate_step_and_side_distance(t_cub *data, t_dda *dda, double ray_dir_x, double ray_dir_y)
{
    if (ray_dir_x < 0)
    {
        dda->step_x = -1;
...
double calculate_wall_distance(t_cub *data, t_dda *dda, double ray_dir_x, double ray_dir_y)
{
    if (dda->side == 0)
        return (dda->map_x - data->var.position_x + (1 - dda->step_x) / 2) / fabs(ray_dir_x);
    else
        return (dda->map_y - data->var.position_y + (1 - dda->step_y) / 2) / fabs(ray_dir_y);
}

void calculate_wall_height(t_render *render, int window_height, double perp_wall_dist)
{
    ...
    render->draw_end = (render->draw_end >= window_height) ? window_height - 1 : render->draw_end;
}
...
void render_raycasting(t_data *img, t_cub *data)
{
    ...
    while (x < window_width)
    {
        ...
        double perp_wall_dist = calculate_wall_distance(data, &dda, ray_dir_x, ray_dir_y);
        ...
        int wall_color = (dda.side == 0) ? RED : DARK_RED; // Red or Darker Red
