...
void apply_screen_overlay(t_data *img, int factor)
{
    ...
    while (i < SCREEN_WIDTH)
    {
        ...
        {
            color = *(int*)(img->addr + (j * img->line_length + i * (img->bits_per_pixel / 8)));
            darkened = (color & ALPHA_MASK) | 
                      ((color & RED_MASK) / factor) | 
                      ((color & GREEN_MASK) / factor) | 
                      ((color & BLUE_MASK) / factor);
